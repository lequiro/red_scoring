"""
Pipeline de scoring predictivo sobre features.csv.

  ⚠ Las features de historial [A] NO se usan (definen es_sano → fuga).
  Experimentos (red_morosos quedó fuera del pipeline):
  ...
  Validación: Leave-One-Out CV, métrica AUC-ROC.
...
# ⚠ FUGA DE TARGET: [A] se deriva de la serie de `situacion` que define es_sano.
# Se lista solo como referencia de qué columnas evitar; ningún modelo lo usa.
FEATS_A_LEAKY = ["peor_situacion", "situacion_final", "n_periodos_mora",
                 "racha_max_mora", "tendencia", "n_transiciones", "volatilidad"]
FEATS_B = ["log_monto", "n_periodos", "comunidad_escala"]              # escala
FEATS_C = ["degree_monto", "pct_vecinos_sanos", "media_pond_sit_vecinos"]  # vecindad
FEATS_D = ["pct_monto_banco", "tiene_fogAr", "n_entidades_distintas"]  # entidad
...
def rf_importancia(X, y, feature_names):
    """Random Forest sobre todo el set — importancias de Gini."""
    X_imp = SimpleImputer(strategy="median").fit_transform(X)
    rf = RandomForestClassifier(n_estimators=300, max_depth=6, random_state=config.SEED)
    rf.fit(X_imp, y)
...
def plot_auc_comparacion(resultados):
    ...
    ax.legend()
    fig.tight_layout()
    path = config.FIGURES_DIR / "scoring_auc_comparacion.png"
...
def plot_feature_importance(imp_df):
    ...
    fig.tight_layout()
    path = config.FIGURES_DIR / "scoring_feature_importance.png"
    fig.savefig(str(path), dpi=150)
    plt.close(fig)
    print(f"PNG: {path}")
...
def plot_score_distribucion(df_scores, nombre_modelo):
    ...
    fig.tight_layout()
    path = config.FIGURES_DIR / "scoring_scores_distribucion.png"
    fig.savefig(str(path), dpi=150)
    plt.close(fig)
    print(f"PNG: {path}")
...
def main():
    ...
    print(f"Cargando {config.FEATURES_CSV} ...")
    df = pd.read_csv(str(config.FEATURES_CSV), index_col=0)
    df.index = df.index.astype(str)  # cuit como string (identificador, no número)
    ...
    imp_df = rf_importancia(df[feats_m1].to_numpy(), y, feats_m1)
    print("\nTop-10 features:")
    ...
    path_csv = config.TABLES_DIR / "scoring_resultados.csv"
    df_scores.to_csv(str(path_csv))
